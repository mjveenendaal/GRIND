...
#                    idList = [point for point in allPointsId]
                ...
                temp2 = []
                coefficient = 10**(5-len(str(pointExist[0].x)))
                for p in temp1:
                    if p['xypoint'].distance(center)/radius < p['imp']:
                        temp2.append(p['id'])
                total = int(len(temp2) * percentage)
                allx = np.array([x*coefficient for x in Point_Cloud.objects.filter(xypoint__within = polygon, id__in = temp2).values_list('x', flat=True).order_by('imp')])[:total]
                ally = np.array([y*coefficient for y in Point_Cloud.objects.filter(xypoint__within = polygon, id__in = temp2).values_list('y', flat=True).order_by('imp')])[:total]
                allz = np.array([z*coefficient for z in Point_Cloud.objects.filter(xypoint__within = polygon, id__in = temp2).values_list('z', flat=True).order_by('imp')])[:total]
